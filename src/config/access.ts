 ...
 *
 * ⚠ `technical`, `legal` and `compliance` have **no** workspace. Their route
 * trees are described in ROLE_PERMISSION_MATRIX § 11 as "page design được đề
 * xuất… không phải các page đã implement". They are not invented here.
 */
export type WorkspaceId = "customer" | "sales" | "manager" | "admin";
 ...
 *
 * ⚠ NEEDS CLARIFICATION (U-07): ROLE_PERMISSION_MATRIX § 10 defines only
 * `dd:*`, `ncnda:*` and `kyc:*` permission *names* for technical/legal/
 * compliance and states § 12.1 that the read/write/approve matrix is not
 * available. No frontend surface consumes them, so those roles are granted
 * nothing here. Fail-closed, not fail-open.
 */
 ...
 * that have no workspace (technical / legal / compliance — U-07), rather than
