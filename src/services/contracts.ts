 ...
 */
export interface SalesWorkspaceService { overview(query?: {from?:string;to?:string}): Promise<SalesOverview>; conversionReport(query?: {from?:string;to?:string;groupBy?:string}): Promise<SalesConversionReport>; activityReport(query?: {from?:string;to?:string;dealId?:string}): Promise<SalesActivityReport>; forecastReport(query?: {from?:string;to?:string}): Promise<SalesForecastReport>; listLeads(query?: Record<string,string|number|boolean|undefined>): Promise<SalesLeadPage>; getLead(id:string): Promise<SalesLeadDetail>; qualifyLead(id:string, body:SalesLeadQualifyRequest): Promise<SalesLeadQualifyResponse>; listTasks(query?: Record<string,string|number|boolean|undefined>): Promise<TaskPage>; getTask(id:string): Promise<ActivitySummary>; updateTask(id:string, body:TaskUpdateRequest): Promise<{activityId:string;updatedAt:string}>; listActivities(dealId:string, query?:Record<string,unknown>): Promise<ActivityPage>; createActivity(body:ActivityCreateRequest): Promise<{activityId:string;dealRevision:number;updatedAt:string}>; listCustomers(query?:Record<string,unknown>): Promise<CustomerPage>; getCustomer(id:string): Promise<CustomerDetail>; listLeadNotifications(query?: {unreadOnly?:boolean;limit?:number}): Promise<import('@/models/salesWorkspace').LeadNotification[]>; markLeadNotificationRead(id:string): Promise<{notificationId:string;read:boolean}>; completeLeadFollowUpTask(id:string): Promise<{taskId:string;status:string;updatedAt:string}>; }
