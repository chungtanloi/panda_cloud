...
export const mockApi: ApiClient = {
  ...
  manager: {
    ...
    projects: async () => ({ items: [], nextCursor: null, isDone: true }),
  ...
  admin: {
    overview: async () => ({ users: { countsByStatus: {}, countsByType: {} }, memberships: { countsByRole: {}, countsByStatus: {} }, governance: { webhookEventCountsByStatus: {} }, ddConfiguration: { templateCount: 0, versionCountsByStatus: {} } }),
    users: async () => ({ items: [], nextCursor: null, isDone: true }),
    user: async (id: string) => ({ userId: id, email: "", fullName: "", userType: "customer", status: "active", lastLoginAt: null, updatedAt: new Date().toISOString(), revision: 1, memberships: [] }),
    updateUser: async (id: string, body: import('@/models/admin').AdminUserUpdateRequest) => ({ userId: id, email: "", fullName: "", userType: "customer", status: body.status, lastLoginAt: null, updatedAt: new Date().toISOString(), revision: 2, memberships: [] }),
    roles: async () => ({ roles: [], readOnly: true }),
    health: async () => ({ api: { status: "unknown" }, convex: { status: "unknown", schemaVersion: 0 }, serverTime: new Date(0).toISOString() }),
    auditLogs: async () => ({ items: [], nextCursor: null, isDone: true }),
    auditLog: async (id: string) => ({ auditId: id, actorUserId: null, actorType: "system", action: "unknown", resourceType: "unknown", resourceId: id, organizationId: null, createdAt: new Date(0).toISOString(), beforeData: null, afterData: null }),
    events: async () => ({ items: [], nextCursor: null, isDone: true }),
    event: async (id: string) => ({ eventId: id, provider: "unknown", externalEventId: id, eventType: "unknown", status: "ignored" as const, attemptCount: 0, lastAttemptAt: null, nextAttemptAt: null, retryExhausted: false, receivedAt: new Date(0).toISOString(), processedAt: null, lastErrorSummary: null }),
    organizations: async () => ({ items: [], nextCursor: null, isDone: true }),
    organization: async (id: string) => ({ organizationId: id, legalName: "", displayName: "", organizationType: "customer", registrationNumber: null, countryCode: null, websiteUrl: null, status: "active", archivedAt: null, updatedAt: new Date().toISOString(), revision: 1, membershipCount: 0 }),
    createOrganization: async (body: import('@/models/admin').AdminOrganizationCreateRequest) => ({ organizationId: "org_mock", legalName: body.legalName, displayName: body.displayName, organizationType: body.organizationType, registrationNumber: body.registrationNumber ?? null, countryCode: body.countryCode ?? null, websiteUrl: body.websiteUrl ?? null, status: "prospect", archivedAt: null, updatedAt: new Date().toISOString(), revision: 1 }),
    updateOrganization: async (id: string, body: import('@/models/admin').AdminOrganizationUpdateRequest) => ({ organizationId: id, legalName: body.legalName ?? "", displayName: body.displayName ?? "", organizationType: "customer", registrationNumber: null, countryCode: null, websiteUrl: null, status: body.status ?? "active", archivedAt: null, updatedAt: new Date().toISOString(), revision: 2 }),
    memberships: async () => ({ items: [], nextCursor: null, isDone: true }),
    membership: async (orgId: string, membershipId: string) => ({ membershipId, organizationId: orgId, user: { userId: "", email: "", fullName: "", userType: "customer", status: "active", lastLoginAt: null, updatedAt: new Date().toISOString(), revision: 1 }, role: "customer" as import('@/models/auth').MembershipRole, status: "active", updatedAt: new Date().toISOString(), revision: 1 }),
