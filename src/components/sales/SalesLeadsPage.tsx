  ...
  return (
    <WorkspacePage eyebrow="Sales / CRM" title="Leads" description="Assessment leads assigned by Manager appear here with their paid context and linked deal.">
      ...
      {loading ? <p className="text-ink-dim">Loading leads…</p> : error ? <p role="alert" className="text-red-300">{error.message}{error.correlationId ? ` Support id: ${error.correlationId}` : ""}</p> : leads.length === 0 ? <div className="surface-card p-8 text-center text-ink-dim">No leads found for this status.</div> : <>
        <div className="surface-card overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b border-white/10 text-xs uppercase tracking-wider text-ink-dim"><th className="p-4">Lead</th><th className="p-4">Source</th><th className="p-4">Assessment</th><th className="p-4">Deal</th><th className="p-4">Status</th><th className="p-4">Updated</th></tr></thead><tbody>{leads.map((lead) => <tr className="border-b border-white/5" key={lead.leadId}><td className="p-4"><Link className="text-accent hover:underline" href={`/sales/leads/${lead.leadId}`}>{lead.summary ?? "Untitled lead"}</Link><p className="text-xs text-ink-dim">{lead.organizationId ? "Organization linked" : "No organization linked"}</p></td><td className="p-4 text-ink-dim">{lead.source} / {lead.persona ?? "—"}</td><td className="p-4 text-xs text-ink-dim">{lead.assessmentSessionId ? <><span className="text-accent">Paid assessment</span><br />{lead.assessmentSessionId}</> : "—"}</td><td className="p-4 text-xs text-ink-dim">{lead.dealId ?? "—"}</td><td className="p-4"><span className="rounded-full border border-accent/40 px-2 py-1 text-xs text-accent">{lead.status}</span></td><td className="p-4 text-ink-dim">{formatDate(lead.updatedAt)}</td></tr>)}</tbody></table></div>
