...
export function SalesLeadDetail({ id }: { id: string }) {
  ...
  const completeFollowUp = async () => {
    if (!lead?.followUpTask) return;
    setBusy(true); setMessage(null);
    try {
      const result = await api.salesWorkspace.completeLeadFollowUpTask(lead.followUpTask.taskId);
      setLead((current) => current ? { ...current, followUpTask: current.followUpTask ? { ...current.followUpTask, status: result.status } : null } : current);
      setMessage("Follow-up task completed.");
    } catch (cause) { setMessage(normalizeError(cause).message); } finally { setBusy(false); }
  };
