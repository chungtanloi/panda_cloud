...
import type { AdminMembership, AdminUserSummary, AdminAuditRecord, AdminIntegrationEvent } from "@/models/admin";
import type { AdminUserUpdateRequest } from "@/models/admin";
import { assignableRoles, canEditMembership, type MembershipRole, type AuthProfile } from "@/models/auth";
...
export function AdminApiView({ kind }: { kind: Kind }) {
  ...
  return <WorkspacePage eyebrow={eyebrow} title={title} description="Operational webhook metadata without provider payloads."><EventTable items={pageItems<AdminIntegrationEvent>(state.data)} /></WorkspacePage>;
}

 export function AdminUserDetailView({ userId }: { userId: string }) { const { profile } = useAuth(); const { state, run } = useAsync(() => api.admin.user(userId), { immediate: [] }); if (state.status === "loading" || state.status === "idle") return <WorkspacePage eyebrow="Admin / Identity" title="User" description="Identity and membership administration."><LoadingState label="Loading user" /></WorkspacePage>; if (state.status === "error") return <WorkspacePage eyebrow="Admin / Identity" title="User" description="Identity and membership administration."><ErrorState error={state.error} onRetry={() => void run()} /></WorkspacePage>; const memberships = Array.isArray(state.data.memberships) ? state.data.memberships : []; return <WorkspacePage eyebrow="Admin / Identity" title={state.data.fullName || state.data.email} description={state.data.email}><dl className="grid gap-3 rounded-2xl border border-line bg-surface p-5 text-sm sm:grid-cols-2"><Info label="Status" value={state.data.status} /><Info label="Type" value={state.data.userType} /><Info label="Last login" value={state.data.lastLoginAt ? new Date(state.data.lastLoginAt).toLocaleString() : "—"} /><Info label="User ID" value={state.data.userId} /></dl><section className="mt-5 rounded-2xl border border-line bg-surface p-5"><h2 className="font-semibold text-ink">Memberships and roles</h2><div className="mt-3 space-y-3 text-sm">{memberships.map((membership) => <MembershipRoleEditor key={`${membership.membershipId ?? membership.organizationId}-${membership.role}`} membership={membership} profile={profile} />)}</div>{memberships.length === 0 ? <p className="mt-3 text-sm text-ink-dim">No memberships returned.</p> : null}</section></WorkspacePage>; }

function MembershipRoleEditor({ membership, profile }: { membership: AdminMembership; profile: AuthProfile | null }) {
  ...
  const editable = canEditMembership(profile, membership);
  const save = async () => {
    if (!dirty || reason.trim().length < 3) return;
    setBusy(true); setMessage(null);
    try {
      const updated = await api.admin.updateMembership(membership.organizationId, membership.membershipId, { role, reason: reason.trim(), expectedRevision: membership.revision });
      setRole(updated.role); setReason(""); setMessage("Role updated and audited.");
    } catch (cause) { setMessage(normalizeError(cause).message); }
    finally { setBusy(false); }
  };
  return <div className="rounded-xl border border-line p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium text-ink">{membership.organizationId}</p><p className="text-xs text-ink-dim">Current role: {membership.role} · {membership.status}</p></div>{editable ? <AdminActionGuard><label className="text-sm text-ink">Role<select aria-label={`Role for ${membership.organizationId}`} value={role} onChange={(event) => setRole(event.target.value as MembershipRole)} className="ml-2 min-h-11 rounded-lg border border-line bg-canvas px-3 py-2"><option value={membership.role}>{membership.role}</option>{options.map((candidate) => <option key={candidate} value={candidate}>{candidate}</option>)}</select></label></AdminActionGuard> : null}</div>{editable ? <AdminActionGuard><div className="mt-3 flex flex-wrap items-end gap-3"><label className="min-w-64 flex-1 text-xs text-ink-dim">Reason<input aria-label={`Reason for ${membership.organizationId}`} value={reason} onChange={(event) => setReason(event.target.value)} maxLength={500} className="mt-1 min-h-11 w-full rounded-lg border border-line bg-canvas px-3 py-2 text-sm text-ink" placeholder="Why is this role changing?" /></label><button type="button" className="min-h-11 rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-accent-fg disabled:cursor-not-allowed disabled:opacity-50" disabled={!dirty || reason.trim().length < 3 || busy} onClick={() => void save()}>{busy ? "Saving…" : "Save role"}</button></div></AdminActionGuard> : null}{message ? <p role="status" className="mt-2 text-xs text-ink-dim">{message}</p> : null}</div>;
}

export function AdminAuditDetailView({ auditId }: { auditId: string }) { const { state, run } = useAsync(() => api.admin.auditLog(auditId), { immediate: [] }); if (state.status === "loading" || state.status === "idle") return <WorkspacePage eyebrow="Admin / Governance" title="Audit record" description="Redacted, read-only audit detail."><LoadingState label="Loading audit record" /></WorkspacePage>; if (state.status === "error") return <WorkspacePage eyebrow="Admin / Governance" title="Audit record" description="Redacted, read-only audit detail."><ErrorState error={state.error} onRetry={() => void run()} /></WorkspacePage>; const record = state.data; return <WorkspacePage eyebrow="Admin / Governance" title={record.action} description="Redacted, read-only audit detail."><dl className="grid gap-3 rounded-2xl border border-line bg-surface p-5 text-sm sm:grid-cols-2"><Info label="Resource" value={`${record.resourceType} · ${record.resourceId}`} /><Info label="Actor" value={record.actorUserId ?? record.actorType} /><Info label="Organization" value={record.organizationId ?? "—"} /><Info label="Created" value={new Date(record.createdAt).toLocaleString()} /></dl><ChangeData label="Before" data={record.beforeData} /><ChangeData label="After" data={record.afterData} /></WorkspacePage>; }
...
function arrayField<T>(value: unknown, field: string): T[] { if (!value || typeof value !== "object") return []; const candidate = (value as Record<string, unknown>)[field]; return Array.isArray(candidate) ? candidate as T[] : []; }
function ChangeData({ label, data }: { label: string; data: AdminAuditRecord["beforeData"] }) { return <section className="mt-5 rounded-2xl border border-line bg-surface p-5"><h2 className="font-semibold text-ink">{label}</h2>{data && Object.keys(data).length > 0 ? <dl className="mt-3 space-y-2 text-sm">{Object.entries(data).map(([key, value]) => <div key={key} className="flex justify-between gap-3"><dt className="text-ink-dim">{key}</dt><dd className="text-right text-ink">{String(value ?? "—")}</dd></div>)}</dl> : <p className="mt-3 text-sm text-ink-dim">No redacted {label.toLowerCase()} data returned.</p>}</section>; }
