...
import { MEMBERSHIP_ROLES, type MembershipRole } from "@/models/auth";
...
function MembershipRoleEditor({ membership }: { membership: AdminMembership }) {
  ...
  return <div className="rounded-xl border border-line p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium text-ink">{membership.organizationId}</p><p className="text-xs text-ink-dim">Current role: {membership.role} · {membership.status}</p></div><AdminActionGuard><label className="text-sm text-ink">Role<select aria-label={`Role for ${membership.organizationId}`} value={role} onChange={(event) => setRole(event.target.value as MembershipRole)} className="ml-2 min-h-11 rounded-lg border border-line bg-canvas px-3 py-2"><option value={membership.role}>{membership.role}</option>{MEMBERSHIP_ROLES.filter((candidate) => candidate !== membership.role).map((candidate) => <option key={candidate} value={candidate}>{candidate}</option>)}</select></label></AdminActionGuard></div><AdminActionGuard><div className="mt-3 flex flex-wrap items-end gap-3"><label className="min-w-64 flex-1 text-xs text-ink-dim">Reason<input aria-label={`Reason for ${membership.organizationId}`} value={reason} onChange={(event) => setReason(event.target.value)} maxLength={500} className="mt-1 min-h-11 w-full rounded-lg border border-line bg-canvas px-3 py-2 text-sm text-ink" placeholder="Why is this role changing?" /></label><button type="button" className="min-h-11 rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-accent-fg disabled:cursor-not-allowed disabled:opacity-50" disabled={!dirty || reason.trim().length < 3 || busy} onClick={() => void save()}>{busy ? "Saving…" : "Save role"}</button></div></AdminActionGuard>{message ? <p role="status" className="mt-2 text-xs text-ink-dim">{message}</p> : null}</div>;
}
