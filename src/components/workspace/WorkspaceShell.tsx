...
import { getUserRole, hasPermission, navigationByRole } from "@/config/access";
import { useAuth } from "@/controllers/AuthContext";
import { cn } from "@/lib/cn";
import { RoleGuard } from "./RoleGuard";

export function WorkspaceShell({ workspace, children }: { workspace: UserRole; children: React.ReactNode }) {
  return <RoleGuard workspace={workspace}><Shell workspace={workspace}>{children}</Shell></RoleGuard>;
}

function Shell({ workspace, children }: { workspace: UserRole; children: React.ReactNode }) {
  const pathname = usePathname();
  const router = useRouter();
  const { user, logout } = useAuth();
  const [open, setOpen] = useState(false);
  const nav = navigationByRole[workspace].filter((item) => !item.permission || hasPermission(user, item.permission));
